...
> **Verification note:** Exact counts below reflect the design intent for the 50-SKU rebuild. Run `Rscript R/03_verify.R` to confirm actual counts from the database.

## Database overview

The Cinderhaven product master database contains 9 tables: product_master (50 rows), sku_costs (50), stores (902), distribution_log, chargebacks, promotions, price_history, scan_data, and retailer_requirements (29). The database represents a fictional specialty food company at approximately $25 million in trailing twelve-month revenue with 50 SKUs across 3 product lines (Artisan Sauces, Specialty Condiments, Pantry Staples), selling through 4 contracted retailers (Walmart, Costco, UNFI, Whole Foods) across 902 stores.

## Intentional defects and their real-world basis

### GTIN-14 check digit errors

**What was done:** Misaligned the mod-10 check digit on approximately 10% of SKUs (5 of 50).
...
**What this enables in the analysis:** The chargeback-to-defect linkage that is the centerpiece of the report. Invalid check digits generating chargebacks over 18 months, traceable to a specific field in a specific record.

### UPC-12 check digit errors

**What was done:** Misaligned the mod-10 check digit on approximately 2 of 50 SKUs.
...
**What was done:** Populated the brand_owner field with the string "NA" on approximately 19% of SKUs (~9 of 50).
...
**What was done:** Left case weight, length, width, or height blank on approximately 32% of SKUs (~16 of 50).
...
**What was done:** Set approximately 90% of SKUs (~45 of 50) to "Not Registered" or "Registered - Incomplete." Only ~5 SKUs are "Registered - Complete."
...
**What was done:** Varied serving size strings across multiple distinct formats (e.g., "2 tbsp (30 mL)," "30ml," "2 tablespoons," "30 milliliters," "2T").
...
**What was done:** Seeded chargeback event counts and dollar amounts to follow a Pareto distribution: a small number of SKUs generate 50% of chargeback dollars, and roughly half the catalog has zero chargebacks.
...
**What was done:** Modeled the gap between store authorization date and first scan date with a quality-dependent delay. SKUs with lower data quality scores receive longer delays, calibrated to produce a roughly 3x spread between the worst and best quality tiers.
