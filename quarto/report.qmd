...
`r n_50pct` SKUs generate half of that bill. `r n_80pct` generate 80%. The concentration is not extreme because the underlying defects concentrate in a subset: 27 of 50 SKUs carry at least one data defect, and those 27 account for the vast majority of chargeback dollars. But some SKUs generate far more than others. Here are the names:
...
## The $111-a-month problem nobody sees
...
CHP-AS-005, Classic Tomato Basil, carries $7,976 in annualized chargebacks spread across all six retailers. That works out to roughly $111 per retailer per month. Each month, at each retailer, $111 appears on the settlement statement as a compliance deduction. Not large enough to flag. Not unusual enough to investigate. Not connected to the other five retailers generating the same charge for the same reason.

A hundred and eleven dollars. Six retailers. Thirty-six months. One wrong digit.

This is the structural problem. The chargebacks persist because the defects persist. The defects persist because nobody has time to find them. Nobody has time to find them because the cost of each individual defect is too small to surface through normal business processes. The total is $228,845 a year. The individual units are invisible. The system that would make them visible does not exist yet. Part 3 of this report describes what that system looks like.
...
The gap between "blocked" and "ready" for every SKU in the catalog is the same: barcode check digits. The work is measured in hours, not weeks. 27 failing SKUs are not 27 product development problems. They are `r n_skus` data entry tasks. The difference between $0 in expansion-ready revenue and `r ds(total_ttm_rev)` in expansion-ready revenue is approximately `r round(total_fix_hours)` hours of clerical work. That is the most underspent `r round(total_fix_hours)` hours in the company.

## The SKU you can't afford to ignore

CHP-DG-007, Trail Mix Premium, is the ninth-best-selling product in the Cinderhaven catalog. $896,803 in trailing twelve-month revenue. 212 stores across every channel. Velocity of 14.1 units per store per week. It represents 2.7% of company revenue and $655,280 in annual gross margin.

It fails every retailer's readiness check. Zero of six.

The product carries three defects: an invalid GTIN-14 check digit, an invalid UPC check digit, and missing case dimensions. It has generated 54 chargeback events over 36 months — $56,502 in penalties, roughly $1,570 a month. Twenty-three of its 212 store authorizations have been revoked. Those slots are gone.

The chargebacks break down by reason: late deliveries account for 35.7%, short shipments for 28.6%, damaged goods for 18.3%, and label and barcode fines for 17.4%. The data defect — the barcode — drives only a fraction of the chargeback dollars directly. What it drives is the deauthorization risk. A retailer's automated readiness check does not distinguish between a $1,570-a-month product and a $15-a-month product. It checks the barcode. The digit fails. The product is flagged.

Fixing CHP-DG-007 takes 50 minutes. Correct the GTIN check digit. Correct the UPC check digit. Measure and enter the case dimensions. When that is done, $18,834 a year in chargebacks is reduced, 23 lost store authorizations stop compounding, and the #9 SKU moves from zero of six retailers passing to a candidate for all six.

The same pattern repeats across the top of the catalog:

| Rank | Product | Revenue | DQ score | Chargebacks | Retailers passing |
|------|---------|---------|----------|-------------|-------------------|
| 1 | Truffle Mushroom Sauce | $1.2M | 100.0 | $0 | 6 of 6 |
| 2 | Extra Virgin Olive Oil | $1.1M | 83.3 | $6,941 | 1 of 6 |
| 3 | Maple Syrup Grade A | $1.0M | 83.3 | $0 | 1 of 6 |
| 4 | Wildflower Honey | $1.0M | 100.0 | $2,319 | 6 of 6 |
| 5 | Balsamic Fig Glaze | $1.0M | 83.3 | $2,671 | 1 of 6 |
| 6 | Smoky Chipotle BBQ Sauce | $1.0M | 83.3 | $5,155 | 1 of 6 |
| 7 | Sun-Dried Tomatoes | $961k | 83.3 | $2,280 | 3 of 6 |
| 8 | Lemon Herb Chimichurri | $919k | 100.0 | $0 | 6 of 6 |
| 9 | Trail Mix Premium | $897k | 83.3 | $26,462 | 0 of 6 |
| 10 | Carolina Gold BBQ | $885k | 100.0 | $0 | 6 of 6 |

Four of the top ten are clean — perfect data quality scores, no chargebacks, passing all six retailers. Six are not. The difference is not commercial attention. Everyone at Cinderhaven knows what Trail Mix Premium sells. Nobody knows that its GTIN-14 check digit is wrong. Those are two different kinds of knowing, and only the first one happens naturally.

The risk of leaving CHP-DG-007 unfixed is not the $18,834. The risk is that a retailer runs the check. Walmart does not send a chargeback for a readiness failure. Walmart sends a deauthorization. And when a $900,000 SKU — one that already fails every retailer it ships to — loses shelf space at its largest account, the conversation is not with the data team. It is with the board.
...
The instinct is to assume the big sellers would have been cleaned up by now. The assumption is wrong because it confuses commercial attention with data attention. Everyone at Cinderhaven knows that Trail Mix Premium sells $897,000 a year. Nobody at Cinderhaven knows that its GTIN-14 check digit is wrong and its case dimensions are blank. Those are two different kinds of knowing, and only the first one happens naturally.

Data entry is clerical work. It happens at launch, when somebody has 20 minutes between other tasks, and it never happens again. Nobody revisits the product master after a SKU is selling. The record freezes at whatever state it was in on the day someone typed it. A $1.2 million SKU and a $319,000 SKU both get one pass through data entry. The difference is that the clean $1.2 million SKU happened to have its digits typed correctly on day one. The $897,000 SKU did not.
...
"Label / barcode fine" chargebacks totaled $137,041 over 36 months, annualized to $45,680 a year. Combined with pricing errors ($5,466 annualized), data-related chargebacks account for 22% of the total bill. The remaining chargebacks trace to fulfillment operations and are outside the scope of this data audit.
...
| **Total** | **27 unique** | **15 hrs** | **$51,147 data-defect** |
...
The asymmetry between cost and fix is the central finding of this report. Not the $228,845 total. Not the Pareto concentration. Not the near-universal retailer readiness failure at the strictest retailers. The asymmetry. The fact that a $34 million company is generating $51,147 a year in data-related penalties because nobody has spent 15 hours on data entry. The fact that the same defects have been present since the day each SKU was entered, accumulating charges in amounts too small to trigger investigation and too steady to ever stop on their own.
...
The result is near-uniform: `r pct_fail_all`% of SKUs fail every retailer. The failure is driven by barcode validation (GTIN-14 and UPC check digits). Brand owner and country of origin are complete and correct. Case dimensions are missing for 19 of 50 SKUs, contributing to readiness failures at retailers that require them. The few SKUs that pass at individual retailers do so because those retailers require fewer barcode fields, not because the data is clean.
...
Non-barcode data fields are mostly complete: brand owner, country of origin, and case weight are correct across the catalog. Case dimensions are missing for 19 of 50 SKUs — the only non-barcode defect category with measurable scope. This means the dataset does not support analysis of missing-field defects or quality-tier variation — limitations the narrative acknowledges directly.
