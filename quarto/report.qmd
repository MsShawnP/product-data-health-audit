...
ds <- function(x) {
  ...
  else paste0("$", formatC(round(x), big.mark = ",", format = "d"))
}
ds_short <- function(x) {
  if (abs(x) >= 1e6) sprintf("$%.1fM", x / 1e6)
...
quality_tiers <- smf |>
  mutate(tier = cut(data_quality_score,
    quantile(data_quality_score, c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE),
    labels = c("Worst 25%", "Below average", "Above average", "Best 25%"),
    include.lowest = TRUE)) |>
  group_by(tier) |>
...
deauth_top <- smf |> filter(data_quality_score > dq_median) |>
  summarise(rate = mean(deauth_rate, na.rm = TRUE), n = n(),
            n_any = sum(deauth_count > 0, na.rm = TRUE))
deauth_ratio <- round(deauth_bottom$rate / deauth_top$rate, 1)
shelf_loss_cost <- round((deauth_bottom$rate - deauth_top$rate) *
...
n_unknown_source <- sum(smf$updated_by == "unknown", na.rm = TRUE)
n_entry_paths <- n_distinct(smf$updated_by)
```
...
chp02_gtin_events <- cbe |>
  ...
  nrow()
chp02_gtin_pct <- round(100 * chp02_gtin_events / max(chp02_events, 1))
```
...
The contrast is instructive. ```{r chp02-vs-chp20}
#| include: false
...
#| include: false
es <- smf |> count(updated_by, name = "n") |> arrange(desc(n))
es_desc <- paste(
...
The obvious hypothesis is that Artisan Sauces routes through a better data entry process. It doesn't. The cross-tab between product line and entry source shows Artisan Sauces actually over-indexes on quality_mgr, ```{r qm-cb}
#| include: false
...
proddata_cb_18mo <- cbe |>
  filter(grepl("Missing product data", reason, ignore.case = TRUE)) |>
  summarise(t = sum(amount, na.rm = TRUE)) |> pull(t)
