...
top_rev_annual  <- round(top_rev$chargeback_total * 12 / 36)
top_rev_monthly <- round(top_rev$chargeback_total / 36)
top_cb_events   <- cbe |> filter(sku == top_cb$sku) |> nrow()
top_cb_annual   <- round(top_cb$chargeback_total * 12 / 36)

n_upc_fail <- sum(!smf$upc_valid, na.rm = TRUE)

data_defect_pct <- cbe |>
  mutate(is_data = !is.na(triggered_by_field)) |>
...
barcode_annual <- round(barcode_cb_36mo * 12 / 36)
...
These two deadlines land on a company whose product master would fail basic validation at nearly every retailer today. `r n_sku_fail_all` of `r n_skus` SKUs fail every retailer's required-field checks. `r n_retailer_zero_pass` of `r n_retailers` retailers have a 0% pass rate. The handful of SKUs that pass at one or two retailers do so because those retailers require fewer fields — not because the data is clean. `r ds(total_ttm_rev)` in trailing twelve-month revenue rides on data that would not survive the onboarding process at most retailers if submitted fresh.
...
Cinderhaven's `r n_retailers` retailers deducted `r ds(total_cb_36mo)` in chargebacks from settlement payments over the past 36 months. Annualized, that's `r ds(annual_cb)` a year in revenue that left the building before it reached the bank account.

The chargebacks arrive under five headings: label and barcode fines, pricing errors, damaged goods, late deliveries, and short shipments. Chargebacks split into two buckets. Data defects — invalid barcodes, pricing errors — account for `r data_defect_pct`% of the bill. Fulfillment operations — short shipments, late deliveries, receiving discrepancies, damaged goods — account for the other `r 100 - data_defect_pct`%. This audit scopes to the data side. Not because the logistics side is smaller. Because the data side is bounded, clerical work — check digit corrections measured in hours per SKU, with a quantifiable effort and a predictable outcome. Fixing a short-ship rate is an operational overhaul with neither. Both matter. This report addresses the one where the scope, effort, and return are known before the work starts.
...
The barcode defect is not concentrated in a few problem SKUs. It is the catalog's default state. `r n_upc_fail` of `r n_skus` SKUs carry invalid UPC check digits. `r n_gtin_fail` carry invalid GTIN-14s. No SKU in the catalog has both a valid GTIN and a valid UPC. Every SKU that ships to a retailer with an invalid barcode is a candidate for a compliance penalty every time the barcode is scanned, validated, or reconciled against a data feed.

The chargebacks are spread across `r n_retailers` retailers and five reason categories. "Label / barcode fine" is the largest single category, but "Pricing error," "Damaged goods," "Late delivery," and "Short shipment" also contribute. The data-related reasons (label/barcode and pricing) account for `r data_defect_pct`% of the total bill. The remaining chargebacks trace to fulfillment operations and are outside the scope of this data audit.

The uniformity of the defect explains two things. First, why the chargeback spread across SKUs is relatively narrow: the top chargeback SKU (`r ds(top_cb$chargeback_total)`) is only about three times the bottom. Every SKU has the same type of defect, so the variation comes from shipping volume and retailer mix, not from defect severity. Second, why the fix is so concentrated: Correct the barcodes and the data-defect chargebacks stop. No long tail of miscellaneous data problems to chase — the defect is narrow and near-universal, so the fix is one category of work applied across the catalog. The logistics chargebacks continue until the fulfillment operations that cause them change. This audit does not pretend to address both. It addresses the portion where the effort is quantifiable, the timeline is bounded, and the return is measurable before the work begins.
...
`r n_sku_fail_all` of Cinderhaven's `r n_skus` SKUs fail every retailer's required-field check. Not a single product in the catalog has both a valid GTIN-14 and a valid UPC, so no product could be submitted to a retailer that checks both without data work first. The expansion pipeline is blocked by barcode validation failures.
...
If Cinderhaven wants to pitch a line extension at any retailer, and at `r ds(total_ttm_rev)` in revenue and growing, that pitch is coming, every SKU needs its data fixed before the conversation starts. Not during the conversation. Before. A retailer's category team does not fix vendor data. They evaluate what's submitted. If the submission fails their automated checks, the conversation ends before a human being ever sees the product.
...
It is also among the largest sources of chargeback cost. `r top_rev_events` chargeback events over 36 months. `r ds(top_rev$chargeback_total)` in penalties deducted from settlement payments across multiple retailers. Not clustered in one bad quarter. Not triggered by one bad shipment. `r top_rev_events` events spread across 36 months, because the same barcode defects trigger the same automated validation failures at the same retailers, month after month.
...
The defects are not complex. The GTIN-14 check digit is wrong. The UPC check digit is wrong. Label and barcode fines account for `r top_rev_barcode_pct`% of `r top_rev$sku`'s chargeback dollars. The rest comes from pricing errors and operational categories.

The product passes `r top_rev_retailers_pass` of `r n_retailers` retailers' required-field checks today. It is authorized at `r top_rev_stores` stores and its data would not survive the onboarding process at most of them if it were submitted fresh. It was authorized before the checks existed or before they were enforced at their current stringency. It survives on inertia, not on data quality.
...
`r if (top10_any_pass == 0) "None" else top10_any_pass` of the top 10 SKUs pass any retailer's full check. The `r ds(top10_total_rev)` in revenue at the top of the catalog, `r top10_rev_pct`% of the company, rides on data that would fail most retailer onboarding processes.
...
Nearly every SKU in the catalog carries the same defect. `r n_upc_fail` of `r n_skus` have invalid UPC check digits. `r n_gtin_fail` also carry invalid GTIN-14 check digits. No SKU has both valid. The basic completeness checks — brand owner, case dimensions, country of origin, weights — all pass, which is why the data quality score reads `r mean_dq_all` for every SKU. But completeness is not the same as correctness. The barcodes are present and the right length. They just fail check-digit validation, which is what retailers actually run.
...
data_defect_annual <- round((total_cb_36mo * data_defect_pct / 100) * 12 / 36)
...
"Label / barcode fine" chargebacks totaled `r ds(barcode_cb_36mo)` over 36 months, annualized to `r ds(barcode_annual)` a year. Combined with pricing errors (which also trace to data fields), data-related chargebacks account for `r data_defect_pct`% of the total bill. The remaining chargebacks trace to fulfillment operations and are outside the scope of this data audit.

```{r fix-action-table}
#| results: asis
fix_tbl <- tibble(
  `Fix action` = c("Fix UPC check digits", "Fix GTIN-14 check digits",
...
![Monthly chargeback dollars over the 36-month window.](`r chart("15_monthly_chargeback_trend")`){fig-alt="Line chart of monthly chargeback dollars showing a roughly flat trend at about $19,000 per month."}
...
The analysis runs against the Cinderhaven Data Platform's dbt mart layer (Postgres) containing `r length(raw)` tables: `r db_desc`.
...
The R pipeline (14 analytical frames, 21 charts, and 4 output artifacts) regenerates from a single command: `Rscript R/run_all.R`. The pipeline reads from the dbt mart layer, builds canonical data frames, generates all charts and the Excel workbook, and renders the Quarto report and dashboard. Total execution time: under two minutes.
:::
...
Chargeback annualization: 36-month totals are multiplied by 12/36 to produce annual run rates. This assumes the monthly chargeback rate is stationary. Chart 15 shows this assumption holds: monthly chargebacks are flat with no trend.
