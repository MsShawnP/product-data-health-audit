...
All figures in this report are computed from Cinderhaven's transaction data covering January 2023 through December 2025 — `r n_chargeback_months` months of chargebacks, scan data, and distribution records.
...
top10_tbl_simple <- smf |>
  ...
  transmute(SKU = sku, Product = product_name,
            ...
            `36-mo chargebacks` = ds_short(chargeback_total))
...
CHP-DG-007, Trail Mix Premium, is the sixth-best-selling product in the Cinderhaven catalog. `r ds(dg007$ttm_revenue)` in trailing twelve-month revenue. `r dg007$auth_count` stores across every channel. It represents `r ds(dg007$annual_gross_margin)` in annual gross margin.

It fails every retailer's readiness check. Zero of six.

The product carries three defects: an invalid GTIN-14 check digit, an invalid UPC check digit, and missing case dimensions. It has generated `r dg007_events` chargeback events over 36 months — `r ds(dg007$chargeback_total)` in penalties. `r dg007$deauth_count` of its `r dg007$auth_count` store authorizations have been revoked. Those slots are gone.
...
top10_dq <- smf |>
  ...
  transmute(Rank = row_number(), Product = product_name,
            ...
            `Chargebacks (36mo)` = ds_short(chargeback_total),
...
The instinct is to assume the big sellers would have been cleaned up by now. The assumption is wrong because it confuses commercial attention with data attention. Everyone at Cinderhaven knows that Trail Mix Premium sells `r ds(dg007$ttm_revenue)` a year. Nobody at Cinderhaven knows that its GTIN-14 check digit is wrong and its case dimensions are blank. Those are two different kinds of knowing, and only the first one happens naturally.
...
## 15 hours against `r ds(annual_cb)`
...
Label and barcode fine chargebacks, combined with pricing errors, account for `r data_defect_pct`% of the total bill — `r ds(annual_cb - non_data_annual)` a year traced directly to data defects. The remaining `r 100 - data_defect_pct`% traces to fulfillment operations and is outside the scope of this data audit.
...
| **Total** | **27 unique** | **15 hrs** | **$`r fmt_d(data_defect_annual)` data-defect** |
...
![Monthly chargeback dollars over the 36-month window.](`r chart("15_monthly_chargeback_trend")`){fig-alt="Line chart of monthly chargeback dollars showing a roughly flat trend at about $12,200 per month."}
