...
All figures in this report are computed from Cinderhaven's transaction data covering `r cb_start_date` through `r cb_end_date` — `r n_chargeback_months` months of chargebacks, scan data, and distribution records.

These two deadlines land on a company whose product master is split: `r n_skus - n_sku_fail_all` of `r n_skus` SKUs pass readiness checks at every retailer, but `r n_sku_fail_all` fail all `r n_retailers`. The failing SKUs share the same defect profile — invalid barcode check digits and missing case dimensions — and no retailer's requirements are lenient enough to let them through. Retailer pass rates range from `r min(rr_stats$pass_rate)`% to `r max(rr_stats$pass_rate)`%, which means the catalog is not uniformly broken, but the broken portion is completely blocked. `r ds(total_ttm_rev)` in trailing twelve-month revenue includes products on both sides of that line.
...
Cinderhaven's `r n_retailers` retailers deducted `r ds(total_cb_36mo)` in chargebacks from settlement payments over the past `r n_chargeback_months` months. Annualized, that's `r ds(annual_cb)` a year in revenue that left the building before it reached the bank account.
...
Fifty-six dollars. Six retailers. `r n_chargeback_months` months. One wrong digit.
...
Deauthorization analysis: the catalog-wide mean deauthorization rate is `r mean_deauth_pct`%. The three-tier score distribution (see Data quality scoring below) enables basic quality-tier comparison; a real engagement with more varied defect types would support richer differential analysis.
