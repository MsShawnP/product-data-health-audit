...
`r n_sku_fail_all` of Cinderhaven's `r n_skus` SKUs fail every retailer's required-field check. `r n_barcode_fail` of `r n_skus` products in the catalog carry invalid barcodes`r if (n_both_barcode_fail == n_barcode_fail) paste0(" — every one failing both GTIN-14 and UPC validation") else paste0(", and ", n_both_barcode_fail, " fail both GTIN-14 and UPC validation")`, so no product could be submitted to a retailer that checks both without data work first. The expansion pipeline is blocked by barcode validation failures.
...
It fails every retailer's readiness check. Zero of `r n_retailers`.
