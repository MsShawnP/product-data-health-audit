...
If Cinderhaven wants to pitch a line extension at any retailer, and at `r ds(total_ttm_rev)` in revenue and growing, that pitch is coming, every SKU in the pitch needs clean data before the conversation starts — and 27 of 50 do not have it. Not during the conversation. Before. A retailer's category team does not fix vendor data. They evaluate what's submitted. If the submission fails their automated checks, the conversation ends before a human being ever sees the product.
...
Deauthorization analysis: the catalog-wide mean deauthorization rate is `r mean_deauth_pct`%. Data quality scores fall into three tiers (66.7, 83.3, and 100), enabling a basic quality-tier comparison. In a real engagement with more varied defect types, a richer differential analysis would isolate the data-quality contribution to deauthorization with greater precision.

Cost model: this report uses annualized chargebacks (`r ds(annual_cb)`) as the primary cost metric. Stalled-launch and shelf-loss costs are not quantified because the three-tier data quality distribution provides directional support for differential analysis, though the limited number of tiers constrains precision.

Data quality scoring: each SKU is scored on 8 binary checks (GTIN-14 valid, UPC valid, brand owner present, country of origin present, case weight plausible, case dimensions present, OneWorldSync complete, serving size standardized). Score = (checks passed / 8) x 100. SKUs score between 66.7 and 100. Clean SKUs (23 of 50) score 100. Single-defect SKUs score 83.3. Multi-defect SKUs score 66.7. The failing checks are barcode-related (GTIN-14 and UPC) and case dimensions; all other checks pass universally. The three-tier score distribution supports quality-tier analysis — a limitation that would not exist in a real engagement where defect types are more varied.
