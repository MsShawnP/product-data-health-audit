...
Within that subset, the defects cluster. `r n_upc_fail` SKUs carry invalid UPC check digits, and the same `r n_gtin_fail` carry invalid GTIN-14s. All of them also lack case dimensions; the remaining case-dimension gaps sit on otherwise-clean SKUs. The overlap means the highest-defect SKUs carry three problems each, while the single-defect SKUs carry one. Data quality scores reflect this directly: each of the six checks a SKU fails costs `r dq_step` points, so a clean SKU scores 100, a single-defect SKU `r round(100 - dq_step, 1)`, and the most-defective score as low as `r dq_min`.

The concentration explains two things. First, why the chargeback spread is narrow: the five costliest SKUs generate `r top5_cb_pct`% of the total bill, and all five carry three defects each. Second, why the fix is so bounded: 15 hours of data entry covers the entire scope. Correct the barcodes on 12 SKUs, enter the case dimensions on 19, and the data-defect chargebacks stop. No long tail of miscellaneous problems to chase — the defects are specific and the SKUs are named.
