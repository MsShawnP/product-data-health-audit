...
stage2_cb   <- round(annual_cb * (stage2_skus / n_skus) * (5 / 3))
stage3_cb   <- round(annual_cb * (stage3_skus / n_skus) * (8 / 3))
...
top_rev_annual  <- round(top_rev$chargeback_total * 12 / 18)
top_rev_monthly <- round(top_rev$chargeback_total / 18)
top_cb_events   <- cbe |> filter(sku == top_cb$sku) |> nrow()
top_cb_annual   <- round(top_cb$chargeback_total * 12 / 18)

n_upc_fail <- sum(!smf$upc_valid, na.rm = TRUE)

data_defect_pct <- cbe |>
  mutate(is_data = !grepl("Late delivery|Short shipment|Damaged goods", reason,
                          ignore.case = TRUE)) |>
...
barcode_annual <- round(barcode_cb_18mo * 12 / 18)
...
all_fail_all <- sum(rrs$overall_pass) == 0
```

# Part 1: The Money

By the end of 2027, every barcode in American retail will change. GS1 Sunrise 2027 transitions the industry from linear barcodes to 2D barcodes built on GS1 Digital Link: a QR code whose foundation is a valid GTIN. Not a GTIN that looks right to a human eye. A GTIN that passes algorithmic validation. `r n_gtin_fail` of Cinderhaven's `r n_skus` SKUs carry GTIN-14s that do not pass. All `r n_upc_fail` carry invalid UPC-12 check digits. Those SKUs cannot participate in the transition until someone corrects the digits.
...
These two deadlines land on a company whose product master would fail basic validation at every retailer today. Not most retailers. Every one. All `r n_skus` SKUs fail all `r n_retailers` retailers' required-field checks. The failure is uniform and total: `r ds(total_ttm_rev)` in trailing twelve-month revenue, 100% of the catalog, rides on data that would not survive the onboarding process at any retailer if submitted fresh.
...
rr_tbl <- rr_stats |>
  ...
  transmute(Retailer = retailer,
            ...
            `Revenue at risk` = ds_short(rev_at_risk(retailer)),
...
These are not projections of what might happen if data quality degrades. They are measurements of the current product master against the current retailer requirements. The data fails now, at every retailer, for every SKU. The only reason the revenue still flows is that nobody has run the audit yet.

The convergence of universal retailer readiness failure, GS1 Sunrise, and FSMA 204 is the wall. Each individually would justify fixing the product master. Together, they create a deadline. A company that reaches 2028 with `r n_gtin_fail` invalid GTINs and `r n_upc_fail` invalid UPCs will not have a data quality problem. It will have a market access problem.

The `r ds(annual_cb)` in annual chargebacks is what dirty data costs when nobody checks. The `r ds(total_ttm_rev)` in at-risk revenue is what it costs when someone does. The GS1 and FSMA transitions are the moment when everyone checks at once.

## The `r ds(annual_cb)` you already pay

Cinderhaven's `r n_retailers` retailers deducted `r ds(total_cb_18mo)` in chargebacks from settlement payments over the past 18 months. Annualized, that's `r ds(annual_cb)` a year in revenue that left the building before it reached the bank account.

The chargebacks arrive under five headings: label and barcode fines, pricing errors, damaged goods, late deliveries, and short shipments. Late deliveries, short shipments, and damaged goods are logistics problems. Label and barcode fines and pricing errors are data problems: charges that trace to wrong or incomplete product records. Those data-related categories account for `r data_defect_pct`% of the chargeback bill.

![Chargeback dollars by reason.](`r chart("14_chargeback_by_reason")`){fig-alt="Horizontal bar chart breaking down chargeback dollars by reason, with data-defect reasons accounting for the majority of dollars."}

`r n_50pct` SKUs generate half of that bill. `r n_80pct` generate 80%. The concentration is not extreme because the underlying defect is universal: every SKU in the catalog carries barcode validation failures, and every SKU generates chargebacks. But some SKUs generate far more than others. Here are the names:
...
The barcode defect is not concentrated in a few problem SKUs. It is the catalog's default state. `r n_upc_fail` of `r n_skus` SKUs carry invalid UPC-12 check digits. `r n_gtin_fail` carry invalid GTIN-14s. Every SKU that ships to a retailer with an invalid barcode is a candidate for a compliance penalty every time the barcode is scanned, validated, or reconciled against a data feed.

The chargebacks are spread across `r n_retailers` retailers and five reason categories. "Label / barcode fine" is the largest single category, but "Pricing error," "Damaged goods," "Late delivery," and "Short shipment" also contribute. The data-related reasons (label/barcode and pricing) account for `r data_defect_pct`% of the total bill. The remainder is logistics.

The uniformity of the defect explains two things. First, why the chargeback spread across SKUs is relatively narrow: the top chargeback SKU (`r ds(top_cb$chargeback_total)`) is only about three times the bottom. Every SKU has the same type of defect, so the variation comes from shipping volume and retailer mix, not from defect severity. Second, why the fix is so concentrated: correct the barcodes and the entire data-defect chargeback bill disappears. There is no long tail of miscellaneous problems to chase.
...
`r deauth_ex$deauth_count` stores out of `r deauth_ex$auth_count` sounds minor. It isn't. It's the signal that the retailer's system has flagged this SKU. The barcode defects that triggered the deauthorizations are identical to the defects still present at the other `r deauth_ex_remaining` stores. `r ds(deauth_ex_cb_total)` in chargebacks over 18 months with no sign of stopping. The deauthorized stores were not the punishment. They were the warning shot. The unfixed barcodes are the loaded gun still pointed at the other `r deauth_ex_remaining`.
...
`r top_rev$sku`, `r top_rev$product_name`, is the #1 revenue SKU in the catalog at `r ds(top_rev$ttm_revenue)`. It has generated `r top_rev_events` chargeback events in 18 months, `r ds(top_rev$chargeback_total)` in total, roughly `r ds(top_rev_monthly)` a month. `r top_rev_events` times, the system flagged the wrong barcode digit, generated a penalty, deducted it from a settlement, and nobody traced it back. Not because tracing it was hard. Because nobody knew to look.
...
Zero of Cinderhaven's `r n_skus` SKUs pass any retailer's required-field check today. Not one product in the catalog could be submitted for a new line review, a planogram expansion, or a regional test without data work first. The entire expansion pipeline is blocked by barcode validation failures.
...
top10_tbl_simple <- smf |>
  ...
  transmute(SKU = sku, Product = product_name,
            ...
            `18-mo chargebacks` = ds_short(chargeback_total))
cat(kable(top10_tbl_simple, format = "pipe", align = c("l", "l", "r", "r")), sep = "\n")
```

`r ds(top10_rev_total)` in revenue from the top 10 SKUs alone, all riding on data that every retailer's system would reject. These products are already generating chargebacks. The risk and the cost are happening simultaneously.

If Cinderhaven wants to pitch a line extension at any retailer, and at `r ds(total_ttm_rev)` in revenue scaling toward $55 million, that pitch is coming, every SKU needs its data fixed before the conversation starts. Not during the conversation. Before. A retailer's category team does not fix vendor data. They evaluate what's submitted. If the submission fails their automated checks, the conversation ends before a human being ever sees the product.

The gap between "blocked" and "ready" for every SKU in the catalog is the same: barcode check digits. The work is measured in hours, not weeks. `r n_skus` failing SKUs are not `r n_skus` product development problems. They are `r n_skus` data entry tasks. The difference between $0 in expansion-ready revenue and `r ds(total_ttm_rev)` in expansion-ready revenue is approximately `r round(total_fix_hours)` hours of clerical work. That is the most underspent `r round(total_fix_hours)` hours in the company.

## The SKU you can't afford to ignore

```{r top-rev-detail}
#| include: false
top_rev_stores <- top_rev$auth_count
top_rev_vel <- round(top_rev$ttm_units / 52 / pmax(top_rev_stores, 1), 1)
top_rev_pct <- round(100 * top_rev$ttm_revenue / total_ttm_rev, 1)
top_rev_gm <- round(top_rev$annual_gross_margin)
top_rev_dq <- top_rev$data_quality_score
top_rev_retailers_pass <- rrs |> filter(sku == top_rev$sku, overall_pass) |> nrow()
```

`r top_rev$sku`, `r top_rev$product_name`, is the best-selling product in the Cinderhaven catalog. `r ds(top_rev$ttm_revenue)` in trailing twelve-month revenue. `r top_rev_stores` stores across every channel. Velocity of `r top_rev_vel` units per store per week. It represents `r top_rev_pct`% of company revenue and `r ds(top_rev_gm)` in annual gross margin. If Cinderhaven has a flagship, this is it.

It is also among the largest sources of chargeback cost. `r top_rev_events` chargeback events over 18 months. `r ds(top_rev$chargeback_total)` in penalties deducted from settlement payments across multiple retailers. Not clustered in one bad quarter. Not triggered by one bad shipment. `r top_rev_events` events spread across 18 months, because the same barcode defects trigger the same automated validation failures at the same retailers, month after month.

```{r top-rev-barcode-share}
#| include: false
top_rev_barcode_events <- cbe |>
  filter(sku == top_rev$sku,
         grepl("Label.*barcode", reason, ignore.case = TRUE)) |>
  nrow()
top_rev_barcode_dollars <- cbe |>
  filter(sku == top_rev$sku,
         grepl("Label.*barcode", reason, ignore.case = TRUE)) |>
  summarise(t = sum(amount, na.rm = TRUE)) |> pull(t)
top_rev_barcode_pct <- round(100 * top_rev_barcode_dollars / max(top_rev$chargeback_total, 1))
```

The defects are not complex. The GTIN-14 check digit is wrong. The UPC-12 check digit is wrong. Label and barcode fines account for `r top_rev_barcode_pct`% of `r top_rev$sku`'s chargeback dollars. The rest comes from pricing errors and operational categories.

The product would fail every retailer's required-field check today. Not one. All `r n_retailers`. It is authorized at `r top_rev_stores` stores and its data would not survive the onboarding process at any of them if it were submitted fresh. It was authorized before the checks existed or before they were enforced at their current stringency. It survives on inertia, not on data quality.
...
top10_tbl <- top10_rev |>
  ...
  transmute(Rank,
            ...
            `Retailers passing` = paste(pass, "of", n_retailers))
cat(kable(top10_tbl, format = "pipe",
          align = c("l", "l", "r", "r", "r", "r")), sep = "\n")
```

![Chargebacks as percentage of gross margin, top 15 SKUs.](`r chart("08_chargeback_pct_of_gross_margin_top15")`){fig-alt="Horizontal bar chart of chargebacks as a share of gross margin for the top-15 SKUs by revenue."}

```{r top10-summary}
#| include: false
top10_any_pass <- sum(top10_pass$pass > 0)
top10_total_rev <- sum(top10_rev$ttm_revenue)
top10_rev_pct   <- round(100 * top10_total_rev / total_ttm_rev)
```

None of the top 10 pass any retailer. The `r ds(top10_total_rev)` in revenue at the top of the catalog, `r top10_rev_pct`% of the company, rides on data that would fail every retailer onboarding process.
...
Fixing `r top_rev$sku` takes `r round(top_rev$est_fix_hours * 60)` minutes. Correct the GTIN check digit. Correct the UPC check digit. When that's done, `r ds(top_rev_annual)` a year in chargebacks is reduced and the #1 SKU moves toward passing retailer readiness checks.

The risk of leaving it unfixed is not the `r ds(top_rev_annual)`. The risk is that a retailer runs the check. Walmart doesn't send a chargeback for a readiness failure. Walmart sends a deauthorization. And when the #1 SKU, `r top_rev_pct`% of company revenue, `r ds(top_rev_gm)` in annual gross margin, loses its largest retailer, the conversation is not with the data team. It is with the board.
...
Every SKU in the catalog has the same problem. All `r n_upc_fail` carry invalid UPC-12 check digits. `r n_gtin_fail` of `r n_skus` also carry invalid GTIN-14 check digits. The data quality score is `r mean_dq_all` for every SKU. There is no variation. No tier of "clean" products to benchmark against. No subset to hold up as proof that the system can work. The entire catalog was entered with the same process, and the entire catalog carries the same defects.

The instinct is to assume the big sellers would have been cleaned up by now. The assumption is wrong because it confuses commercial attention with data attention. Everyone at Cinderhaven knows that `r top_rev$product_name` sells `r ds(top_rev$ttm_revenue)` a year. Nobody at Cinderhaven knows that its GTIN-14 check digit is wrong. Those are two different kinds of knowing, and only the first one happens naturally.

Data entry is clerical work. It happens at launch, when somebody has 20 minutes between other tasks, and it never happens again. Nobody revisits the product master after a SKU is selling. The record freezes at whatever state it was in on the day someone typed it. A `r ds(top_rev$ttm_revenue)` SKU and a `r ds(min(smf$ttm_revenue))` SKU both get one pass through data entry. The `r ds(top_rev$ttm_revenue)` SKU just generates larger chargebacks when the entry is wrong.
...
![True net margin by retailer.](`r chart("03_retailer_net_margin")`){fig-alt="Waterfall chart per retailer, showing Walmart winning gross dollars but not necessarily margin density."}
...
"Label / barcode fine" chargebacks totaled `r ds(barcode_cb_18mo)` over 18 months, annualized to `r ds(barcode_annual)` a year. Combined with pricing errors (which also trace to data fields), data-related chargebacks account for `r data_defect_pct`% of the total bill. The remaining `r ds(non_data_annual)` a year comes from late deliveries, short shipments, and damaged goods — logistics issues outside the scope of a data audit.

```{r fix-action-table}
#| results: asis
fix_tbl <- tibble(
  `Fix action` = c("Fix UPC-12 check digits", "Fix GTIN-14 check digits",
                    "**Total barcode corrections**"),
  SKUs = c(n_upc_fail, n_gtin_fail, NA),
  Time = c(paste(upc_fix_minutes, "min"), paste(gtin_fix_minutes, "min"),
...
![Fix ROI: chargeback savings per hour of effort, by fix action.](`r chart("04_fix_roi")`){fig-alt="Horizontal bar chart of chargeback savings per hour of remediation effort."}

`r round(total_fix_hours)` hours of data entry. That is the entire scope of the barcode cleanup. Case dimensions, brand owner, country of origin, and OneWorldSync registrations are all already complete and correct. The product master's only defects are barcode check digits.

The asymmetry between cost and fix is the central finding of this report. Not the `r ds(annual_cb)` total. Not the Pareto concentration. Not the universal retailer readiness failure. The asymmetry. The fact that a `r ds(total_ttm_rev)` company is losing `r ds(data_defect_annual)` a year in data-related penalties because nobody has spent `r round(total_fix_hours)` hours on data entry. The fact that the same barcode defects have been present since the day each SKU was entered, accumulating charges in amounts too small to trigger investigation and too steady to ever stop.
...
Phase one takes two days. Correct all `r n_upc_fail` invalid UPC-12 check digits and all `r n_gtin_fail` invalid GTIN-14 check digits. Each SKU is a ten-minute fix: open the record, recalculate both check digits using the standard GS1 algorithm, type the correct numbers, save. No other fields need attention — case dimensions, brand owner, country of origin, and OneWorldSync registrations are all complete and correct. When phase one is done, every SKU in the catalog passes all `r n_retailers` retailers' readiness checks. The `r ds(total_ttm_rev)` in at-risk revenue is secured. The first clean settlement statement arrives within 30 to 60 days.
...
The chart reveals a concentrated defect pattern. Invalid UPC-12 affects 100% of SKUs and 100% of revenue. Invalid GTIN-14 affects `r round(100 * n_gtin_fail / n_skus)`% of SKUs. No other defect category registers: brand owner, country of origin, case dimensions, case weight, and OneWorldSync registrations are all complete and correct across the catalog. The product master's problem is narrow — barcode check digits only — but universal.
:::
...
![Retailer item-setup readiness, pass/fail counts.](`r chart("20_retailer_setup_readiness_stacked")`){fig-alt="Stacked bar chart of pass/fail SKU counts per retailer."}

The result is uniform: every SKU fails every retailer. The failure is driven by barcode validation (GTIN-14 and UPC-12 check digits). All other required fields — brand owner, case dimensions, country of origin, OneWorldSync registration — are complete and correct.

The "mean fields short" column matters for planning. Fixing barcode check digits across the catalog moves every SKU from fail to pass at every retailer. The gap between 0% and 100% readiness is a single category of fix.
:::
...
![Monthly chargeback dollars over the 18-month window.](`r chart("15_monthly_chargeback_trend")`){fig-alt="Line chart of monthly chargeback dollars showing a roughly flat trend at about $5,000 per month."}
...
| Stage 2 | `r stage2_skus` | 5 | `r ds(stage2_cb)` |
...
The assumption that matters most is not the defect rate. It's the retailer count. Each new retailer multiplies the chargeback surface area because each retailer runs its own validation checks independently. A SKU with an invalid GTIN generates one charge per retailer per month. At `r n_retailers` retailers, that's `r n_retailers` charges. At 8, it's 8. Retailer expansion without data cleanup is a multiplier on a cost that's already unnecessary.
:::
...
Supply chain operations. Short shipments, late deliveries, and damaged goods account for `r 100 - data_defect_pct`% of chargebacks. These are flagged but not analyzed because they are logistics issues, not data issues.
...
GTIN-14 check digits fail validation on `r n_gtin_fail` of `r n_skus` SKUs. UPC-12 check digits fail on all `r n_upc_fail`. The barcode defect rate is higher than typical real-world rates (where 10–20% is common), which concentrates the narrative on a single defect type. In a real engagement, defect patterns would be more varied.
...
Deauthorization analysis: the catalog-wide mean deauthorization rate is `r mean_deauth_pct`%. Because all SKUs share the same data quality score (`r mean_dq_all`), a quality-tier comparison is not possible with this dataset. In a real engagement with variance in data quality, a differential analysis would isolate the data-quality contribution to deauthorization.

Cost model: this report uses annualized chargebacks (`r ds(annual_cb)`) as the primary cost metric. Stalled-launch and shelf-loss costs are not quantified because the uniform data quality score across the catalog does not support the differential analysis those estimates require.

Data quality scoring: each SKU is scored on 8 binary checks (GTIN-14 valid, UPC-12 valid, brand owner present, country of origin present, case weight plausible, case dimensions present, OneWorldSync complete, serving size standardized). Score = (checks passed / 8) x 100. In this dataset, all SKUs score `r mean_dq_all` because the only failing checks are barcode-related (GTIN-14 and UPC-12); all other checks pass universally. This zero-variance score makes quality-tier analysis impossible — a limitation the narrative addresses honestly.
